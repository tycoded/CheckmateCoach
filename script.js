window.onload = () => {
  ...
  fetch('openings.json').then(res => res.json()).then(data => {
    openingData = data;
    document.getElementById("loadingMessage").style.display = "none";
    document.getElementById("openingSelect").disabled = false;
    document.getElementById("submitOpening").disabled = false;
  });
  ...
  document.getElementById("submitOpening").onclick = () => {
    const select = document.getElementById("openingSelect");
    currentOpening = openingData[select.value];
    if (!currentOpening) {
  ...
  function enforceOpening(mv) {
    ...
    const square = expected.slice(2, 4);
